...
export async function POST(req) {
  try {
    const { name,surname,numDoc,numPhone, email, password,tipoDoc } = await req.json();
    const hashedPassword = await bcrypt.hash(password, 10);
    await connectMongoDB();
    await User.create({ name,surname,numDoc,numPhone, email, password: hashedPassword,tipoDoc });
